...
static const char* get_first_binlog(char* buf_arg)
{
  ...
  if ((length=my_b_gets(index_file, fname, sizeof(fname))) <= 1)
  {
    ...
    DBUG_RETURN("empty binlog index");
  }
  else
  {
    fname[length-1]= 0;                         // Remove end \n
  ...
  {
    DBUG_RETURN("cound not normalize the first file name in the binlog index");
  }
...
/**
  ...
  binlog files. Upon the successful check the supplied domains are removed
  ...
  @param  domain_drop_lex  gtid domain id sequence from lex, may contain dups.
                           Passed as a pointer dynamic array must be not empty.
